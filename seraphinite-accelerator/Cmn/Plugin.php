...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckVer !== '2.27.18' || $lastCheckPackage !== 'Base' )
		{
			$state = Plugin::StateGet();

			if( $lastCheckVer !== '2.27.18' && !isset( $state[ 'changeVerCheck' ] ) )
			{
		...
		if( !$bForce )
		{
			if( $bFirstTimeOnly && $lastCheckVer == '2.27.18' )
				return( Gen::S_FALSE );
...
class Plugin
{
	...
	static function GetAboutPluginContent()
	{
		...
		$urlAboutPluginImg = file_exists( __DIR__ . '/../Images/ProductLogo.png' ) ? add_query_arg( array( 'v' => '2.27.18' ), Plugin::FileUri( '../Images/ProductLogo.png', __FILE__ ) ) : null;
		...
		$url3rdPartySoft = file_exists( __DIR__ . '/../third-party-software.html' ) ? add_query_arg( array( 'v' => '2.27.18' ), Plugin::FileUri( '../third-party-software.html', __FILE__ ) ) : null;
	...
	static function GetAboutVendorContent()
	{
		...
		$urlAboutUsLogoImg = file_exists( __DIR__ . '/../Images/VendorLogo.png' ) ? add_query_arg( array( 'v' => '2.27.18' ), Plugin::FileUri( '../Images/VendorLogo.png', __FILE__ ) ) : null;
	...
	static private function _PrevVer_Check()
	{
		...
		if( empty( $warningVersionInfo ) )
		{
			...
			$verTo = self::_PrevVer_GetInt( '2.27.18' );
			if( $verTo < $verFrom )
...
?>
	...
	static function CmnStyle( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_enqueue_style( Plugin::CmnScriptId( $id ), add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.css' ), array(), '2.27.18' );
	...
	static function CmnScripts( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_register_script( $scrHndId, add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.js' ), $deps, '2.27.18' );
			if( $id == 'Gen' )
				...
				<script>
					(function()
					{
						...
						function _ProcessAnswerAndDeactivateAndClose( answerId, answerUserData )
						{
							...
							sendDataUrl = sendDataUrl.replace( "{PluginVersion}",				encodeURI( "2.27.18" ) );
