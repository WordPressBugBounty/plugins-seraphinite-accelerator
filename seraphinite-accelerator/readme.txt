...
* CloudFlare E-mail decoding for lazy HTML.
...
* CloudFlare E-mail decoding for lazy HTML.
* GEO location: Support for 'Cf-Connecting-IP'.
* HTML markup checker.
* Hidden enabling of object cache is disallowed.
...
* Dropping server/cloud cache immediately after post changing if lazy revalidation id disabled.
* Independence of temporary cache from lazy revalidation.

= 2.29.21 =

Fixes:

* Unable to revalidate cache if the site root contains uppercase characters.
...
* CSS parsing: crash on 'fill:currentColor'.
...
* Vulnerability: Reflected Cross-Site Scripting via the 'seraph_accel_prep' parameter.

= 2.29.18 =

Improvements:

* Ability to cache search queries.
...
* Messed up UI in admin when 'themesflat-addons-for-elementor' is used.
...
* GoDaddy cache clearing.

= 2.29.10 =

Improvements:

* Preventing random content to avoid cache growing: Divi.

Fixes:

* Remote optimization: incorrect site's salt is used.
...
* Processing queue: Dont restarting item while self-learning.

Fixes:

* Multisite: queue processing priority is always by timesmamp.

= 2.29.7 =

Fixes:

* Simple thread of optimization doesn't work with URL with arguments.
...
* Normalize cookies.
* Preventing random content to avoid cache growing: PixelYourSite.
* Responsive of Elementor.

= 2.29.5 =

Improvements:

* Preventing random content to avoid cache growing: Jeg Kit for Elementor.

Fixes:

* Infinite lock at 'at' file.

= 2.29.4 =

Improvements:

* Cache update if templates changed.
* HTML markup checker.
* Lazy HTML for 'script' tags.
* Lazy HTML: batch request.
...
* Server/cloud cache is not cleared after particular update.
...
* Failed status 'writeLrnDone' if self-learning is not enabled.
* HTML: Incorrect parsing of 'a' tags with 'href' containing '<', '>'.

= 2.29.1 =

Improvements:

* Asynchronous tasks: duration of task manager thread.
* Asynchronous tasks: protect singleton task from launching twice if execution is too long.
...
* Cache clearing of Kinsta.
* Car Field Gallery of Vehica theme.
* HTML replacement: escaping placeholders.

Fixes:

* PHP error: Call to undefined function 'class_exist'.
* Wrong order of output buffering flushing while warming up the cache.
...
* Optimized styles after fast optimization are deleted too early if server/cloud cache is stuck.

= 2.28.12 =

Improvements:

* Animation of Nexter Gutenberg Blocks.
* Countdown Timer from Divi.
* Menu of BoldThemes.
